...
public class cluster extends DefaultInternalAction {
    ...
    /*
        ...
        Vec center = new Vec(model, model.getCorral().center());
        ...
        //if (vs.size() > 4)
        // sort cows based on distance to corral's center
        Collections.sort(vs); // sort only big clusters (for small clusters, to sort causes a kind of oscillation)
        
        System.out.println("vs sort "+vs);

        //if (!vs.isEmpty()) 
        //cs.add(vs.remove(0));
            
        // put first cows in cluster
        if (!vs.isEmpty()) cs.add(vs.remove(0));
        ...
        for (Vec v: cs) {
            // place all cows in ref to 0,0
            clusterLocs.add(v.add(center).getLocation(model));
        }
        return clusterLocs;
    }
    */
