...
current host. To run them on several machines there are two
mechanisms.

\subsubsection{Defining the container of each agent}
...
\begin{itemize}
\item create the ant script to run the agents: either press play in the jEdit plugin or type
\begin{verbatim}
   $ cd <your project directory>
   $ <jason dir>/bin/mas2j.sh <the .mas2h file>
\end{verbatim}

  The script has the task \texttt{main-container} to run the main
  container and one task to start each container defined in the
  project (c1 and c2, in the above example).
