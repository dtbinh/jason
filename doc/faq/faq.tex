...
An infrastructure must provide the following services for the MAS:
\begin{itemize}
\item communication (e.g. \saci infrastructure implements it using
  KQML while \jade implements it using FIPA-ACL),
...
\begin{itemize}
\item \emph{Centralised}: this infrastructure runs all agents in the
  same host. It provides fast startup and high performance for systems
  that may be executed in only one computer. It is also useful to test
  and develop the systems. Centralised is the default infrastructure.

\item \emph{Saci}: provides distribution and communication with KQML.

\item \emph{Jade}: provides distribution and communication with
  FIPA-ACL. With this infrastructure all tools available for \jade
  (e.g., Sniffer and Inspector) are also available to monitor and
  inspect \jason agents.

  Since the \jason agents use KQML and \jade uses FIPA-ACL, we opted
  to translate the messages from KQML to FIPA-ACL and vice-versa to
  maintain the AgentSpeak programming the same for all
  infrastructures. The following table resumes the translation of the
  performatives:
  \begin{center}
    \begin{tabular}{ll}
      \hline
...
\subsection{When should I use \saci or \jade infrastructure?}\label{sec:whysaci}
...
\jade infrastructures (or implement a new infrastructure for \jason).
The interoperability with non-\jason agents is achieved by \saci
through KQML communication. An example of communication among \jason
and \saci agents is available at \htlink{Source
...
\subsection{How to start one \jason agent with \texttt{jade.Boot}?}

The \jade agent wrapper should be used to start a \jason agent using
the \texttt{jade.Boot} instead of the \jason IDE. For example, to
start an \jason agent called bob based on the AgentSpeak source of the
file \texttt{auctioneer.asl} run the following command in a shell:
\begin{verbatim}
java jade.Boot bob:jason.infra.jade.JadeAgArch(auctionner.asl)
\end{verbatim}

In case one also wants to start the environment simulator
...
code simply add \code{x(10)} into the agent's belief base through
