...
container will run and the container of where agents will run, as in
...
MAS demo_distributed_jade {
    ...
    agents:
        a at "Main-Container"; // agent a will run the same host as the main container
        b at "c1";             // agent c will run on the host where container c1 will run
        c at "c2" #3;          // ....
        d at "c1";
}
\end{verbatim}
...
\begin{verbatim}
   $ cd <your project directory>
   $ <jason dir>/bin/mas2j.sh <the .mas2h file>
\end{verbatim}

  The script has the task \texttt{Main-Container} to run the JADE main
  container and one task to start each container defined in the
  project (\texttt{c1} and \texttt{c2}, in the above example).
...
\subsubsection{Defining a class that allocate the agents}
...
containers have to be created and how agents are distributed on them.
...
of agents to each container. You might see the code of this class for
details.

To run this system, replace the commands of step 1 of the previous
section as follows (the other steps remains the same).
...
\begin{verbatim}
   ...
   > java -classpath bin\classes;%JASONLIB%\jason.jar jason.mas2j.parser.mas2j  <the .mas2h file>
\end{verbatim}
...
\begin{itemize}
\item an \htlink{SVN}{http://sourceforge.net/docs/B01} client program and
