...
public class marsEnv extends Environment {
	...
	public static final int NGarb = 2;
	...
	public boolean executeAction(String ag, Term action) {
		if (action.equals(ns)) {
			...
			if (cr1[X] == Mid && cr1[Y] == Mid)
				cr1[X]++;
		} else if (action.hasFunctor("moveTowards")) {
			x = (new Integer(action.getTerm(1).toString())).intValue();
			y = (new Integer(action.getTerm(2).toString())).intValue();
			if (cr1[X] < x)
				cr1[X]++;
			else if (cr1[X] > x)
				cr1[X]--;
